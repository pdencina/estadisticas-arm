-- ============================================================
-- ARM Stats · Migración completa v3
...
create table if not exists public.encuentros (
  ...
  fecha                   date not null,
  tipo                    text not null
    check (tipo in ('domingo','miercoles','jueves','sabado','prayer_room','encuentro_global','otro')),
  ...
  voluntarios jsonb not null default '{
    "servicio":0,"tecnica":0,"kids":0,"tweens":0,"worship":0,"cocina":0,
    "rrss":0,"seguridad":0,"sala_bebes":0,"conexion":0,"oracion":0,"merch":0,
    "amor_por_la_casa":0,"sala_sensorial":0,"punto_siembra":0,"cambios":0
  }'::jsonb,
  online jsonb not null default '{
    "acepto_jesus":0,"espectadores_max":0
  }'::jsonb,
...
-- Índices
create index if not exists enc_campus_fecha on public.encuentros(campus_id, fecha desc);
create index if not exists enc_fecha        on public.encuentros(fecha desc);
create index if not exists enc_estado       on public.encuentros(estado);
...
$$;
drop trigger if exists enc_updated_at on public.encuentros;
create trigger enc_updated_at
...
-- Campus: todos los autenticados leen; solo admin_global escribe
drop policy if exists "campus_sel" on public.campus;
drop policy if exists "campus_all" on public.campus;
create policy "campus_sel" on public.campus for select to authenticated using (true);
create policy "campus_all" on public.campus for all    to authenticated
  using (public.mi_rol()='admin_global') with check (public.mi_rol()='admin_global');

-- User profiles
drop policy if exists "prof_sel"  on public.user_profiles;
drop policy if exists "prof_upd"  on public.user_profiles;
drop policy if exists "prof_admin" on public.user_profiles;
create policy "prof_sel"   on public.user_profiles for select to authenticated
  using (id=auth.uid() or public.mi_rol()='admin_global');
create policy "prof_upd"   on public.user_profiles for update to authenticated
  using (id=auth.uid() or public.mi_rol()='admin_global');
create policy "prof_admin" on public.user_profiles for insert to authenticated
  with check (public.mi_rol()='admin_global');

-- Encuentros
drop policy if exists "enc_sel" on public.encuentros;
drop policy if exists "enc_ins" on public.encuentros;
drop policy if exists "enc_upd" on public.encuentros;
drop policy if exists "enc_del" on public.encuentros;
create policy "enc_sel" on public.encuentros for select to authenticated
  using (public.mi_rol()='admin_global' or campus_id=public.mi_campus());
create policy "enc_ins" on public.encuentros for insert to authenticated
  with check (public.mi_rol()='admin_global' or campus_id=public.mi_campus());
create policy "enc_upd" on public.encuentros for update to authenticated
  using (public.mi_rol()='admin_global' or (public.mi_rol()='admin_campus' and campus_id=public.mi_campus()));
create policy "enc_del" on public.encuentros for delete to authenticated
  using (public.mi_rol()='admin_global');

-- Informes: todos leen, admin_global gestiona
drop policy if exists "inf_sel" on public.informes_semanales;
drop policy if exists "inf_all" on public.informes_semanales;
create policy "inf_sel" on public.informes_semanales for select to authenticated using (true);
create policy "inf_all" on public.informes_semanales for all    to authenticated
  using (public.mi_rol()='admin_global') with check (public.mi_rol()='admin_global');

-- ── VISTA: resumen semana actual ──────────────────────────────
create or replace view public.v_semana_actual as
with rango as (
  select
    date_trunc('week', current_date)::date as ini,
...
select
  ...
  c.pais,
  sum(e.total_general) as total_general,
  sum((e.asistencia->>'auditorio')::int) as total_auditorio,
  sum(e.acepto_jesus_presencial + coalesce((e.online->>'acepto_jesus')::int,0)) as total_paj,
  count(*) as num_encuentros
from public.encuentros e
join public.campus c on c.id = e.campus_id
cross join rango r
where e.fecha between r.ini and r.fin
  and e.estado in ('enviado','validado')
group by e.campus_id, c.nombre, c.pais;
...
--
-- 3. Crear adm. de campus:
--    UPDATE user_profiles
--    SET rol='admin_campus', nombre='Mario Muñoz',

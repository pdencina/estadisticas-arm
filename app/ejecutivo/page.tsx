...
import { fmt, fmtFecha } from "@/lib/utils";
import { TendenciaMensual, TablaAnual, CampusBarras, TipoDistribucionChart, PajPorCampus } from "@/components/charts/EjecutivoCharts";
...
export default async function EjecutivoPage() {
  ...
  const { totales, por_anio, por_mes, campus_ranking, por_tipo, crecimiento_yoy } = data;
  ...
  return (
    <div className="page space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 mt-1">Datos desde {primerAnio} · Todos los campus</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="kpi-card border-l-4 border-l-orange-500">
          ...
          <p className="text-3xl font-black tracking-tight" style={{ color: "var(--teal)" }}>{fmt(contador || totales.paj)}</p>
      ...
      {(crecimiento_yoy.encuentros_pct !== 0 || crecimiento_yoy.asistentes_pct !== 0) && (
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">Crecimiento vs año anterior ({new Date().getFullYear() - 1} → {new Date().getFullYear()})</h3>
      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            <p className="text-[10px] text-gray-400 uppercase font-bold">Campus activos</p>
            <p className="text-lg font-black mt-1">{campus_ranking.length}</p>
          </div>
        </div>
      </div>
